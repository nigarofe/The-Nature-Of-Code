...
class NeuralNetwork {
    ...
    mutateThreshold() {
        this.threshold = random(-intervalo, intervalo);
    }

    calculateError() {
        this.error = 0;
        for (let x = 0; x < width; x++) {
            this.error += Math.pow(answer[x] - this.answers[x], 2);
        }
        return this.error;
    }

    getCopy() {
        let copy = new NeuralNetwork(this.layersQuantity, this.networkHeight);
        copy.w = this.w;
        copy.b = this.w;
